...
pkgs.stdenv.mkDerivation rec {
  ...
  src = builtins.path { path = ./.; name = "ca-bundle-etc-passwd-etc-group-sudo-su"; };
  ...
  installPhase = ''
    ...
    cp "${src}"/ca-bundle-etc-passwd-etc-group-sudo-su.sh $out

    install \
    -m0755 \
    $out/ca-bundle-etc-passwd-etc-group-sudo-su.sh \
    -D \
    $out/bin/ca-bundle-etc-passwd-etc-group-sudo-su

    patchShebangs $out/bin/ca-bundle-etc-passwd-etc-group-sudo-su

    wrapProgram $out/bin/ca-bundle-etc-passwd-etc-group-sudo-su \
    ...
    echo 'sshd:x:999:999:::/bin/sh' >> $out/etc/passwd
...
EOF
    ...
    cp $out/etc/passwd $out/etc/shadow
