...
if [[ ${is_local} ]]; then
  ...
  nix build --refresh .#oci-xorg-pkgsstatic-xclock \
  && podman load < result
else
  echo 'Remote building'
  nix build --refresh github:ES-Nix/nix-oci-image/nix-static-minimal#oci-xorg-pkgsstatic-xclock \
  && podman load < result
fi
...
localhost/oci-podman-xorg-pkgsstatic-xclock:latest
xhost -
